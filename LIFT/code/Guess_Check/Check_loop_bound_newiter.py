...
def load_config(config_path='config.yaml'):
    default_config = {
        ...
        'logging': {
            'log_dir': 'logs/prev_ce_feedback',
            'log_filename': 'all_history.log'
        },
        'output': {
            'result_filename': 'results/prev_ce_feedback/all_history.txt'
        },
        'verification': {
...
class BoogieVerifier:
    ...
    def _terminate_process_tree(self):
        ...
        try:
            ...
            self._active_process = None
            self._active_pgid = None
        # Best-effort fallback: brute-force kill by process name to avoid orphans
    ...
    def run_verification(self, k_file_name):
        ...
        try:
            ...
            return 3, None, None
        try:
            ret = self._analyze_results(k_file_name)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Loop bound verification')
    parser.add_argument('--config', '-c', default='config.yaml',
                        help='Path to configuration file (default: config.yaml)')
    parser.add_argument('--llm-type_lb', '-lb', type=int,
                        help='Override loop bound LLM type')
    ...
    k_induction_max = config['verification'].get('k_induction_max', 3)
    ...
    error_count = 0
    result = []
    #log_filename = 'prev_ce_feedback0_deepseek.log'
    ...
    for filename, inputfile in files_to_process:
        ...
        # Maximum 5 iterations, exit on successful verification
        while(iter_back < max_conj_iterations and verification_ret != 1):
            ...
            #prompt_type = 2
            (llm_answer, infer_time, token_usage) = openai_gen_answer(0, llm_type, boogie_code, conj_or_lex, prompt_type, feed_back_message, ce, repeat_notice=repeat_notice, code_type='Boogie', if_prompt_ce=if_prompt_ce, if_prompt_hint=if_prompt_hint)
            #llm_answer = "assume(i >= z + 1 && i >= y + z + 2);"
            #infer_time = 0
            ...
            while(k <= k_induction_max):
                ...
                try:
                    ...
                    if verification_ret == 1:
                        ...
                        if k == 1:
                            ...
                            # ce is already extracted in _analyze_results
                        ...
                        logger.info(f"ICE Runtime error with the loop bound: {llm_answer}, file: {full_filename}, k: {k}, infer_time: {infer_time}, feed_back_iter: {iter_back}, tokens: {token_usage['total_tokens']}")
                        if k == 1:
                            prompt_type = 1
                            previous_llm_answer.append([llm_answer, prompt_type, None])
                        break
                ...
                if k == k_induction_max:
                    ...
                    if verification_ret == 1:
                        ...
                        logger.info(f"Verification succeeded with the loop bound: {llm_answer}, file: {full_filename}, const_part: 3, invariant: {invariant}, verification_time: {verifier.time}, infer_time: {infer_time}, feed_back_iter: {iter_back}, tokens: {token_usage['total_tokens']}")
                        success_count += 1
                    else:
            ...
            iter_back += 1
        prompt_type = 0
        ce = None  # Reset counterexample for lexical loop
        while(verification_ret != 1 and iter_back < max_lex_iterations):
            ...
            #llm_answer = openai_gen_answer(llm_type, boogie_code, prompt_type, feed_back_message)
            (llm_answer, infer_time, token_usage) = openai_gen_answer(0, llm_type, c_code, conj_or_lex, prompt_type, feed_back_message, ce, repeat_notice=repeat_notice, code_type='C', if_prompt_ce=if_prompt_ce, if_prompt_hint=if_prompt_hint)
            llm_answer = remove_empty(llm_answer)
            #feed_back_message[0] = llm_answer
            ...
            try:
                ...
                logger.info(f"Klexical template generation error with the loop bound: {llm_answer}, file: {full_filename}, infer_time: {infer_time}, feed_back_iter: {iter_back}, tokens: {token_usage['total_tokens']}")
                prompt_type = 1
                previous_llm_answer.append([str(llm_answer), prompt_type, None])
                iter_back += 1 
                continue
